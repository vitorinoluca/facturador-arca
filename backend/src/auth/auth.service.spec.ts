...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('rechaza un email que ya existe', async () => {
      ...
      await expect(service.register('a@a.com', 'password123')).rejects.toThrow(
        ConflictException,
      );
    });
    ...
    it('hashea la contraseña antes de guardar', async () => {
      ...
      userRepo.save.mockImplementation(
        async (u) => ({ id: '1', ...u }) as User,
      );
      refreshTokenRepo.save.mockImplementation(async (t) => t as RefreshToken);
      verificationTokenRepo.save.mockImplementation(
        async (t) => t as VerificationToken,
      );
    ...
    it('devuelve un access token y un refresh token, y guarda solo el hash del refresh', async () => {
      ...
      const savedRefresh = refreshTokenRepo.save.mock
  ...
  describe('login', () => {
    it('rechaza un email inexistente', async () => {
      userRepo.findOneBy.mockResolvedValue(null);
      await expect(service.login('nadie@a.com', 'x')).rejects.toThrow(
    ...
    it('rechaza una contraseña incorrecta', async () => {
      ...
      await expect(service.login('a@a.com', 'incorrecta')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('rechaza una cuenta de solo-Google (sin contraseña)', async () => {
      ...
      await expect(service.login('a@a.com', 'lo-que-sea')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  });
  ...
  describe('loginWithGoogle', () => {
    it('crea una cuenta nueva si no existe ni por googleId ni por email', async () => {
      userRepo.findOneBy.mockResolvedValue(null);
      userRepo.save.mockImplementation(
        async (u) => ({ id: '1', emailVerified: false, ...u }) as User,
      );
      refreshTokenRepo.save.mockImplementation(async (t) => t as RefreshToken);
  ...
  describe('resetPassword', () => {
    it('rechaza un token vencido', async () => {
      ...
      await expect(
        service.resetPassword('viejo', 'nuevaPass123'),
    ...
    it('rechaza un token ya usado', async () => {
      ...
      await expect(
        service.resetPassword('reusado', 'nuevaPass123'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('actualiza la contraseña y revoca todas las sesiones activas', async () => {
      ...
      expect(verificationTokenRepo.update).toHaveBeenCalledWith(
        { id: 'vt-1' },
