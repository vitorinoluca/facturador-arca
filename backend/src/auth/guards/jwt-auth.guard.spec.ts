...
describe('JwtAuthGuard', () => {
  ...
  it('rechaza sin cookie de access_token', async () => {
    await expect(guard.canActivate(contextWithCookie())).rejects.toThrow(
  ...
  it('rechaza un token inválido o expirado', async () => {
    ...
    await expect(guard.canActivate(contextWithCookie('feo'))).rejects.toThrow(
      UnauthorizedException,
    );
  });
  ...
  it('rechaza un token válido si el usuario ya no existe (cuenta borrada)', async () => {
    ...
    await expect(
      guard.canActivate(contextWithCookie('valido')),
