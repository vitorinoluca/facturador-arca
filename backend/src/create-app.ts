...
// Toda la configuración de la app, sin el listen() — la comparten el servidor
// tradicional (main.ts, para local/Render/Railway/Fly) y la función serverless de
// Vercel (api/index.ts), que nunca llama a listen().
export async function createApp(): Promise<INestApplication> {
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription(
      'Facturación electrónica (ARCA/WSFEv1) para monotributistas',
    )
    .setVersion('1.0')
    .addBearerAuth()
    .build();
